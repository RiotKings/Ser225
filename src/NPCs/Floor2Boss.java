...
public class Floor2Boss extends NPC {
    ...
    private int currentHealth = 13;
    private int maxHealth = 13;

    private float circleCooldown = 0f;
    private static final float CircleInterval = 4f;   // every 4 seconds
    private static final int CircleBulletCount = 12;  // 12 bullets in the circle
    


    private static final float StopDistance = 5;

    public Floor2Boss(int id, float x, float y) {
        super(id, x, y, new SpriteSheet(ImageLoader.load("Floor2Boss.png"), 44, 76), "STAND_LEFT");
    ...
    protected void performAction(Player player) {
        ...
        if (isAttacking) {
            ...
            circleCooldown -= STEP_DT;
            if (circleCooldown <= 0f) {
            spawnCircularBurst(ex, ey);
            circleCooldown = CircleInterval;
    }
    ...
    private void spawnCircularBurst(float ex, float ey) {
